...
*/
	...
	// Java.registerClass() is only supported on ART for now(201803). it is not compatible under android 4.4, try to use ART.
	/*
	...
	*/
	...
	try {
		...
		quiet_log("registerClass from X509TrustManager >>>>>>>> " + e.message);
	}
	...
	var SSLContext_init = SSLContext.init.overload(
		...
		quiet_log('Overriding SSLContext.init() with the custom TrustManager');
	...
	try {
		...
		quiet_log('OkHTTP 3.x Found');

		CertificatePinner.check.overload('java.lang.String', 'java.util.List').implementation = function() {

			quiet_log('OkHTTP 3.x check() called. Not throwing an exception.');
		}
	...
	try {
		...
		PinningTrustManager.checkServerTrusted.implementation = function() {
			...
			quiet_log('Appcelerator checkServerTrusted() called. Not throwing an exception.');
		}
	...
	try {
		...
		OkHttpClient.setCertificatePinner.implementation = function(certificatePinner) {
			// do nothing
			quiet_log("OkHttpClient.setCertificatePinner Called!");
			return this;
		};
		...
		CertificatePinner.check.overload('java.lang.String', '[Ljava.security.cert.Certificate;').implementation = function(p0, p1) {
			// do nothing
			quiet_log("okhttp Called! [Certificate]");
			return;
		};
		CertificatePinner.check.overload('java.lang.String', 'java.util.List').implementation = function(p0, p1) {
			// do nothing
			quiet_log("okhttp Called! [List]");
			return;
		};
	} catch (e) {
		quiet_log("com.squareup.okhttp not found");
	}
	...
	WebViewClient.onReceivedSslError.implementation = function(webView, sslErrorHandler, sslError) {
		quiet_log("WebViewClient onReceivedSslError invoke");
	...
	WebViewClient.onReceivedError.overload('android.webkit.WebView', 'int', 'java.lang.String', 'java.lang.String').implementation = function(a, b, c, d) {
		quiet_log("WebViewClient onReceivedError invoked");
		return;
	};
	...
	try {
		...
		quiet_log("registerClass from hostnameVerifier >>>>>>>> " + e.message);
	}
	...
	try {
		...
		quiet_log("Xutils hooks not Found");
	}
	...
	***/
	...
	try {
		...
		TrustManagerImpl.verifyChain.implementation = function(untrustedChain, trustAnchorChain, host, clientAuth, ocspData, tlsSctData) {
			quiet_log("TrustManagerImpl verifyChain called");
		...
		quiet_log("TrustManagerImpl verifyChain nout found below 7.0");
	}
	// OpenSSLSocketImpl
	try {
		var OpenSSLSocketImpl = Java.use('com.android.org.conscrypt.OpenSSLSocketImpl');
		OpenSSLSocketImpl.verifyCertificateChain.implementation = function(certRefs, authMethod) {
			quiet_log('OpenSSLSocketImpl.verifyCertificateChain');
		}

		quiet_log('OpenSSLSocketImpl pinning')
	} catch (err) {
		quiet_log('OpenSSLSocketImpl pinner not found');
	}
	// Trustkit
	try {
		var Activity = Java.use("com.datatheorem.android.trustkit.pinning.OkHostnameVerifier");
		Activity.verify.overload('java.lang.String', 'javax.net.ssl.SSLSession').implementation = function(str) {
			quiet_log('Trustkit.verify1: ' + str);
			return true;
		};
		Activity.verify.overload('java.lang.String', 'java.security.cert.X509Certificate').implementation = function(str) {
			quiet_log('Trustkit.verify2: ' + str);
			return true;
		};

		quiet_log('Trustkit pinning')
	} catch (err) {
		quiet_log('Trustkit pinner not found')
	}
	...
	try {
		...
		netBuilder.addPublicKeyPins.implementation = function(hostName, pinsSha256, includeSubdomains, expirationDate) {
			...
			//this invoke obj before addPublicKeyPins? Yes,CronetEngine.Builder
			return this;
		};
